...
export class UsersComponent implements OnInit {
  ...
  deleteUserInfo() {
    this.userService.deleteUserInfo(1).subscribe(data => {
      console.log(data);
    });
  }
   ...
   */
  updateUserInfo() {
    this.userService.updateUserInfo(this.userInfos[0]).subscribe((data: UserInfo) => {
      this.userInfos[0] = data;
    });
  }

  searchUserInfos() {
    this.userService.searchUserInfos('tom').subscribe((data: UserInfo[]) => {
      this.userInfos = data;
    });
  }
