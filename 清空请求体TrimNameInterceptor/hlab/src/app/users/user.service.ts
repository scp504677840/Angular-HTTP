...
export class UserService {
  ...
  private userUrl = 'http://localhost:8080/userInfo';
  ...
  listUsers() {
    return this.http.get<UserInfo[]>(this.usersUrl)
      .pipe(retry(3), catchError(this.handleError('listUsers', [])));
  }
  ...
  saveUserInfo(userInfo: UserInfo): Observable<UserInfo> {
    return this.http.post<UserInfo>(this.userUrl, userInfo, httpOptions)
      .pipe(catchError(this.handleError('saveUserInfo', userInfo)));
  }

  deleteUserInfo(id: number): Observable<{}> {
    const url = `${this.userUrl}/${id}`;
    return this.http.delete(url, httpOptions).pipe(catchError(this.handleError('deleteUserInfo', {})));
  }

  updateUserInfo(userInfo: UserInfo): Observable<UserInfo> {
    httpOptions.headers = httpOptions.headers.set('Authorization', 'my-new-auth-token');
    return this.http.put<UserInfo>(this.userUrl, userInfo, httpOptions)
      .pipe(catchError(this.handleError('updateUserInfo', userInfo)));
  }

  searchUserInfos(term: string): Observable<UserInfo[]> {
    term = term.trim();
    // 如果存在搜索词，请添加安全的URL编码搜索参数
    const options = term ? {params: new HttpParams().set('name', term)} : {};
    return this.http.get<UserInfo[]>(this.usersUrl, options)
      .pipe(catchError(this.handleError<UserInfo[]>('searchUserInfos', [])));
  }
